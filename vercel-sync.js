// Vercel Edge Functions 云端同步模块（零配置方案）
class VercelSync {
    constructor(localDB) {
        ...
        // 本地开发时可以使用 localhost，部署后自动切换到生产域名
        this.apiUrl = window.location.origin + '/api/wallpapers';
    ...
    async initialize() {
        try {
            console.log('✅ Vercel 云端同步已启用（实时同步模式）');
    ...
    // 上传到 Vercel Edge Functions（自动，静默）
